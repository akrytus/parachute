...
Version: 1.32 (9/28/22) - Removed static GPO names to allow imports of different GPO names
...
function FSLogixGPO {
    ...
    $gpourl = "https://parafslogix.blob.core.windows.net/gpo/FSLogix-GPO-Settings.zip" #Exported FSLogix GPO at Parachute Dev Storage
    $GPOfile = "FSLogix-GPO-Settings.zip"
    $GPOPath = $ExtractPath + "\FSLogix-GPO-Backup"
    ...
    #Download Backup GPO Zip File From Azure Dev Storage
    ...
    #Import GPO
    Write-Host "`Importing FSLogix GPO Settings..."
    $BackupID = (Get-ChildItem $GPOPath -Directory).Name
    $XMLFile = $GPOPath + "\" + $BackupID + "\gpreport.xml"
    $XMLData = [XML](get-content $XMLFile)
    $GPOName = $XMLData.GPO.Name   
    Import-GPO -BackupId $BackupID -TargetName $GPOName -Path $GPOPath -CreateIfNeeded
